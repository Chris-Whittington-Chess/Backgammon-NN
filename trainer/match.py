...
def match_cube_action(dist: Sequence[float], a: int, b: int, cube: int,
                      ...
                      post_crawford: bool = False) -> MatchCubeDecision:
    ...
    mwc_nodouble = _mwc_from_game(dist, a, b, cube, g, post_crawford)
    mwc_double_take = _mwc_from_game(dist, a, b, 2 * cube, g, post_crawford)
...
def _mwc_from_game(dist: Sequence[float], a: int, b: int, cube: int,
    ...
    for p, pts in ((p_win_single, 1), (p_win_gammon, 2), (p_win_bg, 3)):
        out += p * mwc(_clamp_away(a - pts * cube), b, g, post_crawford)
    for p, pts in ((p_lose_single, 1), (p_lose_gammon, 2), (p_lose_bg, 3)):
        out += p * mwc(a, _clamp_away(b - pts * cube), g, post_crawford)
    return out
